...
class DoubleCtrlHotkey:
    ...
    def _keyboard_proc(self, n_code: int, w_param: int, l_param: int) -> int:
        ...
        if n_code != _HC_ACTION:
            return int(user32.CallNextHookEx(self._hook_handle, n_code, w_param, l_param))
    ...
    def register(self) -> bool:
        ...
        proc_type = ctypes.WINFUNCTYPE(ctypes.c_long, ctypes.c_int, wintypes.WPARAM, wintypes.LPARAM)
        self._hook_proc = proc_type(self._keyboard_proc)
        module_handle = kernel32.GetModuleHandleW(None)
        self._hook_handle = user32.SetWindowsHookExW(_WH_KEYBOARD_LL, self._hook_proc, module_handle, 0)
